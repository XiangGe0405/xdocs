...
``` bash
docker run -p 5508:6379 --name redis5508 -d redis redis-server 
```

2.数据持久化存方式启动

``` bash
docker run -p 5508:6379 -v D:\Docker\Data\redis\data:/data --name redis -d redis redis-server --appendonly yes --requirepass redis@3.14@q
```
...
--network bridge --ip 172.17.0.3   指定容器ip
```

配置文件方式启动
1.创建配置文件目录存放redis.conf 拷贝下面内容
配置文件内容放到了文件的末尾
...
``` bash
docker exec -ti redis redis-cli #d0b86 容器ID 配置成自己的就可以
